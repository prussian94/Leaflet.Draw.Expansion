...
L.Draw.Arc = L.Draw.SimpleShape.extend({
      ...
      if (this._startLatLng && !this._radiusX) {
        ...
        if (this._tooltip) {
          this._tooltip.updateContent({
            ...
            subtext: `RadiusX: ${this._startLatLng.distanceTo(latlng).toFixed(2)} meters`,
          });
        }
        ...
        if (this._tooltip) {
          this._tooltip.updateContent({
            ...
            subtext: `RadiusY: ${this._startLatLng.distanceTo(latlng).toFixed(2)} meters`,
          });
        }
  ...
  _onMouseDown: function _onMouseDown(e) {
    ...
    if (!this._startLatLng) {
      ...
      this._drawLineX(latlng); // Keep X radius line visible
    } else if (!this._radiusY) {
      this._radiusY = this._startLatLng.distanceTo(latlng);
      this._drawLineY(latlng); // Keep Y radius line visible
      this._drawShape(latlng); // Ensure the shape is drawn with both radii
...
L.Edit.Arc = L.Edit.SimpleShape.extend({
  addHooks: function () {
    ...
    if (this._shape._map) {
      // Store the initial state when edit mode starts
      this._initialState = {
  ...
  _initMarkers: function () {
    ...
    this._createMoveMarker(); // Add the move marker
  ...
  _moveShape: function (newCenter) {
    ...
    // Move the arc's points accordingly
...
L.Arc.addInitHook(function () {
  ...
  this.on('add', function () {
    if (this.editing && this.editing.enabled()) {
      this.editing.addHooks(); // Ensure that addHooks is called when the shape is added to the map
    }
  });
